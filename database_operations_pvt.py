...
def get_security_deposit_data(quarters, submarkets):
    ...
    df = db_manager.execute_query_pandas(query, params=quarters + submarkets)
...
def get_area_leased_by_submarket(quarters):
    ...
    df = db_manager.execute_query_pandas(query, params=quarters)
...
def get_tenant_sector_share_data(submarkets):
    ...
    df = db_manager.execute_query_pandas(query, params=submarkets)
...
# Additional utility functions that might be useful for the private version
